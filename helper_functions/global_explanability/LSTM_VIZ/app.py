...
def playIt():
	...
	csv_test_1 = pd.read_csv('datasets/AAPL.csv')
	csv_test_2 = pd.read_csv('datasets/AAPL.csv')
	csv_test_3 = pd.read_csv('datasets/AAPL.csv')
	csv_test_4 = pd.read_csv('datasets/AAPL.csv')
	

	if(var1.get()=='Select Dataset'):
		messagebox.showinfo("Error", "Select A Series To Plot")
		return
	elif(var1.get()=='aadr.us'):
		data= csv_test_1["Close"]
	elif(var1.get()=='AAPL'):
		data= csv_test_2["Close"]
	elif(var1.get()=='acwx.us'):
		data= csv_test_3["Close"]
	elif(var1.get()=='airr.us'):
		data= csv_test_4['Close']
		print('\n\n** ',data)
		print('\n\n\n*** type(data: )', type(data))
	play=1
	try:
...
w2 = Scale(master, from_=2, to=100)
...
lst1 = ['Select Dataset','aadr.us','AAPL','acwx.us','airr.us']
# lst1 = ['Select Dataset','Stock Price','Sine Curve','Cosine Curve','Random Data', 'Increasing Sales', 'Decreasing Sales']
var1 = StringVar()
...
#     rect=win32gui.GetWindowRect(H	WND)
#     x = rect[0]
#     x1=x+master.winfo_width()
#     y = rect[1]
#     y1=y+master.winfo_height()
#     im=ImageGrab.grab((x,y,x1,y1))
#     im.save("second.jpeg",'jpeg')
 
# root = Tk()
# b = Button(root, text='Screenshot', command=CaptureScreen)
# b.pack()
